		...
		acquire(&ptable.lock);
			...
			for(q = ptable.proc; q < &ptable.proc[NPROC]; q++){
				...
				if(lowP != 0){
					...
					switchuvm(lowP);
					lowP->state = RUNNING;
					// cprintf("cpu %d pid %d priority %d\n", c->apicid, lowP->pid, lowP->priority);
			...
			if(p != 0){
				...
				p->state = RUNNING;
				p->stat.num_run++;
				// cprintf("pname %s cpu %d pid %d queue %d rtime %d lwtime %d\n", p->name, c->apicid, p->pid, p->priority, p->rtime, p->lwtime);
				swtch(&(c->scheduler), p->context);
				switchkvm();
